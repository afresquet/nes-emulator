...
impl OpCode {
    pub fn new(code: u8, mnemonic: Mnemonic, len: u8, cycles: u8, mode: AddressingMode) -> Self {
        Self {
            code,
            mnemonic,
            len,
    ...
    pub fn instruction_map() -> &'static HashMap<u8, OpCode> {
        get_or_init_opcodes_map()
    }
}
...
fn get_or_init_opcodes_map() -> &'static HashMap<u8, OpCode> {
    static OPCODES_MAP: OnceLock<HashMap<u8, OpCode>> = OnceLock::new();

    OPCODES_MAP.get_or_init(|| {
        let mut map = HashMap::new();

        for cpuop in get_or_init_opcodes() {
...
fn get_or_init_opcodes() -> &'static Vec<OpCode> {
    static OPCODES: OnceLock<Vec<OpCode>> = OnceLock::new();
    ...
    OPCODES.get_or_init(|| {
        ...
        vec![
            ...
            /* Branching */
            OpCode::new(0x4c, JMP, 3, 3, NoneAddressing), //hat acts as Immidiate
            OpCode::new(0x6c, JMP, 3, 5, NoneAddressing), //ndirect with 6502 bug
